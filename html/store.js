...
export const store = {
  ...
  onCheckCID(err, res) {
    ...
    utils.invokeContract(
      ...
      (...args) => this.onGetArtistKeyCID(...args),
      this.shader
    )
  },
  ...
  onGetArtistKeyCID(err, res) {
    ...
    utils.invokeContract(`role=artist,action=get_key`, (...args) => this.onGetArtistKeyNoCID(...args), this.shader)
  },
  ...
  loadParams() {
    ...
    utils.invokeContract(`role=manager,action=view_params,cid=${this.state.cid}`, (...args) =>
      this.onLoadParams(...args)
    )
  },
  ...
  loadBalance() {
    utils.invokeContract(`role=user,action=view_balance,cid=${this.state.cid}`, (...args) =>
      this.onLoadBalance(...args)
    )
  ...
  withdrawBEAM() {
    ...
    utils.invokeContract(`role=user,action=withdraw,nMaxCount=0,cid=${this.state.cid}`, (...args) =>
      this.onMakeTx(...args)
    )
  },
  ...
  //
  loadArtists() {
    utils.invokeContract(`role=manager,action=view_artists,cid=${this.state.cid}`, (...args) =>
      this.onLoadArtists(...args)
    )
  },
  ...
  loadArtworks() {
    utils.invokeContract(`role=user,action=view_all,cid=${this.state.cid}`, (...args) => this.onLoadArts(...args))
  },
  ...
  setCurrentPage(page) {
    ...
    visibleArts.forEach((art, idx) => {
      if (!art.bytes) {
        this.loadArtwork(idx + firstIdx, art.id)
      }
    })
  ...
  loadArtwork(idx, id) {
    ...
    utils.invokeContract(`role=user,action=download,cid=${this.state.cid},id=${id}`, (err, res) => {
      this.state.pending_artworks--
  ...
  //
  buyArtwork(id) {
    utils.invokeContract(`role=user,action=buy,id=${id},cid=${this.state.cid}`, (...args) => this.onMakeTx(...args))
  },

  sellArtwork(id, price) {
    utils.invokeContract(`role=user,action=set_price,id=${id},amount=${price},aid=0,cid=${this.state.cid}`, (...args) =>
      this.onMakeTx(...args)
    )
  },

  likeArtwork(id) {
    utils.invokeContract(`role=user,action=vote,id=${id},val=1,cid=${this.state.cid}`, (...args) =>
      this.onMakeTx(...args)
    )
  },

  unlikeArtwork(id) {
    utils.invokeContract(`role=user,action=vote,id=${id},val=0,cid=${this.state.cid}`, (...args) =>
      this.onMakeTx(...args)
    )
  },

  deleteArtwork(id) {
    utils.invokeContract(`role=manager,action=admin_delete,id=${id},cid=${this.state.cid}`, (...args) =>
      this.onMakeTx(...args)
    )
  },
  ...
  //
  addRewards(amount) {
    utils.invokeContract(`role=manager,action=add_rewards,num=${amount},cid=${this.state.cid}`, (...args) =>
      this.onMakeTx(...args)
    )
  },
  ...
  uploadArtwork(file, artist_key) {
    ...
    try {
      ...
      reader.onload = () => {
        ...
        utils.invokeContract(
          `role=manager,action=upload,cid=${this.state.cid},pkArtist=${artist_key},data=${hex}`,
          (...args) => this.onMakeTx(...args)
        )
      }
    } catch (err) {
  ...
  showStats() {
    let total = this.state.arts.length
    let left = total

    for (let idx = 0; idx < total; ++idx) {
      let curridx = idx
      let currid = this.state.arts[curridx].id
      utils.invokeContract(`role=user,action=view_item,cid=${this.state.cid},id=${currid}`, (err, res) => {
        if (err) {
          return this.setError(err)
        }

        if (res.sales.length) {
          let artwork = this.state.arts[curridx]
    ...
    let formatter = function (arr, title, extra) {
      ...
      for (let data of arr) {
        ...
        result = [
          ...
          artwork.title,
          '" by "',
          (artists[artwork.pk_author] || {}).label,
          '" id ',
          data.idx + 1,
        ].join('')
