...
export const store = {
  ...
  start() {
    ...
    Vue.nextTick(() => {
      utils.download("./galleryManager.wasm", (err, bytes) => {
        ...
        this.state.refresh_timer = setInterval(
          () => this.refreshAllData(),
          10000
        );
  ...
  onGetArtistKeyCID(err, res) {
    ...
    utils.invokeContract(
      ...
      (...args) => this.onGetArtistKeyNoCID(...args),
      this.shader
    );
  },
  ...
  showStats() {
    ...
    for (let idx = 0; idx < total; ++idx) {
      ...
      utils.invokeContract(
        `role=user,action=view_item,cid=${this.state.cid},id=${currid}`,
        (err, res) => {
          if (err) {
  ...
  __showStats() {
    ...
    let sliked = formatter(likes, "Most liked:", (artwork) =>
      [artwork.impressions, "likes"].join(" ")
    );
    let slikes = ["Total likes: ", likes_total].join("");
    let ssold = formatter(
      sales,
      "Top sales (one per artwork):",
      (artwork, data) => {
